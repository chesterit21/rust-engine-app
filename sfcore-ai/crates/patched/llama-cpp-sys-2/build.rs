...
fn main() {
    ...
    // PATCH: Allow GGML_BLAS config via env vars
    if let Ok(val) = env::var("GGML_BLAS") {
        config.define("GGML_BLAS", &val);
    }
    if let Ok(val) = env::var("GGML_BLAS_VENDOR") {
        config.define("GGML_BLAS_VENDOR", &val);
    }
