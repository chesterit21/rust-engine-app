## 📝 LOGGING SYSTEM DENGAN ASYNC QUEUE - IMPLEMENTATION

Oke bro, aku paham! Kita butuh:

1. ✅ **Non-blocking logging** - lempar ke background task, gak nunggu selesai
2. ✅ **Queue mechanism** - batch insert, gak flood database
3. ✅ **Separate channel** - gak ganggu traffic utama
4. ✅ **Database table** untuk activity logs
...
## 📁 STEP 3: LOGGING DATA STRUCTURES (`src/logging/types.rs`)
...
## 📁 STEP 4: ASYNC LOGGER WITH QUEUE (`src/logging/logger.rs`)
...
## 📁 STEP 5: MODULE EXPORTS (`src/logging/mod.rs`)
...
## 📁 STEP 6: INTEGRATE TO CONVERSATION MANAGER
...
handle_message()
    ↓
logger.log() ← FIRE & FORGET (non-blocking!)
    ↓
...
✅ **Queued** - Flume MPMC channel (10K capacity)  
✅ **Batched** - Insert 100 logs at once (configurable)  
✅ **Timeout flush** - Max 1 second wait  
✅ **Parallel workers** - 2 threads for DB inserts  
✅ **Backpressure** - Queue full → drop with warning  
✅ **No traffic impact** - Separate from main flow  
...
1. 🔍 Analytics query functions untuk dashboard?
2. 📊 Grafana/Prometheus metrics exporter?
3. 🧹 Auto cleanup old logs (retention policy)?
