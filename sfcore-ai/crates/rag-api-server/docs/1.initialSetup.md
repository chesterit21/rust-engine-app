...
SELECT DISTINCT
    ...
    -- User yang punya akses (owner atau shared)
...
-- Join dengan shared documents
LEFT JOIN "TblDocumentShared" ds 
    ON ds."DocumentID" = d."DocumentID"
    
LEFT JOIN "TblDocumentSharedPrivilidge" dsp 
...
CREATE OR REPLACE FUNCTION search_user_documents(
    ...
    p_document_id INTEGER DEFAULT NULL  -- Optional: filter by specific document
)
RETURNS TABLE (
    chunk_id BIGINT,
    document_id INTEGER,
...
SELECT * FROM get_user_document_ids(1);

-- Test access check
SELECT check_user_document_access(1, 123);
```
