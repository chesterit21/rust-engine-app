    ...
    for (var a = 0; a < listAngka.length; a++) {
        ...
        if (no === 9) {
            ...
            for (var i = 0; i < dt.length; i++) {
                row = row + "<td><button type='button' style='color:#666' onclick='notPickNumber(\"" + dt[i] + "\")' tite='click on here for not pick number " + dt[i] + "' class='btn btn-lg btn-xs btn-pck btpx-" + dt[i] + "'>" + dt[i] + "</button></td>";
            }
            row = row + "</tr>";
            no = 0;
            dt = [];
...
window.setNumberNotInGame = async function (tipe) {
    ...
    if (tipe == 'AKK-X') {
        ...
        if (numb.length == 0 || numb == '') {
            alert('THE ANGKA BELUM DI SET UNTUK DI RANGE, HARAP PILIH ANGKA TERLEBIH DAHULU.');
            return false;
        } else {
        ...
        let numb = $('#Tb3DD').val();
        if (numb.length == 0 || numb == '') {
            alert('THE ANGKA BELUM DI SET UNTUK DI RANGE, HARAP PILIH RANGE ANGKA TERLEBIH DAHULU.');
            return false;
        } else {
            await notPickNumber(numb);
        }
...
window.notPickNumber = async function (numb) {
    ...
    let stat = true;
    await showToastr('Set Angka Range ini.? ', 'SET NOT PICK NUMBER');
    if (stat) {
        await delayedGreeting();
        ...
        $.ajax({
            ...
            success: async function (data) {
                await showToastr('Set Angka Range ini Mati.? [SUKSES]', 'SET NOT PICK NUMBER', 'FROM SERVER');
            },
...
async function getLogResult() {
    ...
    $.ajax({
        ...
        success: async function (data) {
            ...
            populateChartResult(data.logGameDtos.slice(0, 360));
        ...
        if (no === 6) {
            ...
            for (var i = 0; i < dt.length; i++) {
                ...
                if ((baris == 3 && i == 0) || (baris == 8 && i == 0)) {
                    row = row + "<td title='" + dt[i].periode + "'  style='cursor:pointer;color:rgb(236 230 255);background-color : rgb(236 230 255);" + borderColor + "' class='tdp-" + dt[i].periode + "' onclick='changeBackground(\"" + dt[i].periode + "\")'>" + dt[i].log + "</td>";
                }
...
window.checkLogIfSameData = function (currentLog, compareLog) {
    ...
    if (currentLog.as == compareLog.as && currentLog.kop == compareLog.kop) {
        ...
        return "border-bottom:2px solid #25be25";
    }
        ...
        return "border:2px solid #25be25";
    }
    else {
...
window.changeBackground = async function (idx, numbInfo, coloring, simulateInc) {
    // console.log(coloring);
    // console.log(numbInfo);
    ...
    if (numbInfo == undefined || numbInfo == null) {
        ...
        if (simulateInc == undefined || simulateInc == null) {
            ...
            await getDataListByTwoDigitNumberSelected(ds, selectedLog, coloring);
        }
    }
    else {
        if ($(idx).css("background-color") === "rgba(0, 0, 0, 0)") { $(idx).css("background-color", "rgb(232, 157, 192)"); }
        else if ($(idx).css("background-color") === "rgb(232, 157, 192)") { $(idx).css("background-color", "rgb(136, 205, 208)"); }
        else if ($(idx).css("background-color") === "rgb(136, 205, 208)") { $(idx).css("background-color", "rgb(221, 222, 222)"); }
        else if ($(idx).css("background-color") === "rgb(221, 222, 222)") { $(idx).css("background-color", "rgb(198, 246, 210)"); }
        else if ($(idx).css("background-color") === "rgb(198, 246, 210)") { $(idx).css("background-color", "rgb(198, 220, 246)"); }
        else { $(idx).css("background-color", "rgb(0, 0, 0, 0)"); }

    }
}

window.getDataListByTwoDigitNumberSelected = async function (ds, selectedLog, coloring) {
    ...
    for (let a = 0; a < 21; a++) {
        ...
        if (log != undefined) {
            ...
            listNumberByMid.push({ as: 0, createdDate: null, dateResultInGame: null, ekor: 0, gameCode: '-', kepala: 0, kop: 0, logResult: '9999', periode: PeriodeMid });
        }
    }
    ...
    for (let a = 0; a < 21; a++) {
        ...
        if (log != undefined) {
            ...
            listNumberByBack.push({ as: 0, createdDate: null, dateResultInGame: null, ekor: 0, gameCode: '-', kepala: 0, kop: 0, logResult: '9999', periode: PeriodeBack });
        }
    }

    listNumberByFront.sort((a, b) => b.periode - a.periode);
    populateChartLogFront(listNumberByFront, coloring);

    listNumberByMid.sort((a, b) => b.periode - a.periode);
    populateChartLogMid(listNumberByMid, coloring);

    listNumberByBack.sort((a, b) => b.periode - a.periode);
    populateChartLogBack(listNumberByBack, coloring);

    //await autoSaveDataLog(coloring);
}
...
async function populateDataVertical(data) {
    ...
    for (let a = 0; a < data.length; a++) {
        const log = data[a];
        row += "<td>";
        row += "<div class='col-anx' style='border-image: linear-gradient(to right, #ff00cc, #33c5ff);border-image-slice: 1;'>";
        row += "<div class='col-lg-12'><span style='font-weight:500;font-size:14px;color:#222'>" + log.logResultTemplate + "</span></div>";
        row += "<div class='col-lg-12' style='color:#333'>" + log.periode + " - <span style='color:purple;font-size:16px; font-weight:bold'>#" + nomor + "</span></div>";
        ...
        for (let a = 0; a < 21; a++) {
            ...
            try {
                ...
                if (log.logPosition.logFronts != null) {
                    ...
                    if (isready != undefined) {
                        rw += "<td style='background:green;color:green'>" + df[a].logResult + "</td>";
                    } else {
                        if (df[a].dateResultInGame == 'INX') rw += "<td style='background:#666;color:#666'>" + df[a].logResult + "</td>";
            ...
            try {
                ...
                if (log.logPosition.logMids != null) {
                    ...
                    if (isready != undefined) {
                        rw += "<td style='background:yellow;color:yellow'>" + dm[a].logResult + "</td>";
                    } else {
                        if (dm[a].dateResultInGame == 'INX') rw += "<td style='background:#666;color:#666'>" + dm[a].logResult + "</td>";
            ...
            try {
                ...
                if (log.logPosition.logBacks != null) {
                    ...
                    if (isready != undefined) {
                        rw += "<td style='background:crimson;;color:crimson'>" + db[a].logResult + "</td>";
                    } else {
                        if (db[a].dateResultInGame == 'INX') rw += "<td style='background:#666;color:#666'>" + db[a].logResult + "</td>";
...
async function tambahNote(tipe) {
    ...
    let bindNotes = async function (data) {
        ...
        for (let a = 0; a < data.length; a++) {
            const note = data[a];
            let row = "<ul style='color:yellowgreen'><i>*# </i>" + note.note + "</ul>";
            divNote.append(row);
        }
    }
}

async function autoSaveDataLog(coloring) {
    if (coloring == null || coloring == undefined) return false;

    let dsf = sessionStorage.getItem('logSelectedFront');
    dsf = JSON.parse(dsf);

    let dsm = sessionStorage.getItem('logSelectedMid');
    dsm = JSON.parse(dsm);

    let dsb = sessionStorage.getItem('logSelectedBack');
    dsb = JSON.parse(dsb);


    var gameCode = $('#hidCode').val();
    var gameId = $('#hidId').val();
    var gamePeriode = $('#hidPeriode').val();

    var lastAs = $('#hidAsx').val();
    var LastKop = $('#hidKopx').val();
    var lastKep = $('#hidKepx').val();
    var lastEkor = $('#hidEkorx').val();

    let listLog = [];
    for (let i = 0; i < dsf.length; i++) {
        const log = dsf[i];
        let dto = restrukturNumberHitGame(log, 'AS-KOP-KEP', gameCode, gameId, gamePeriode, lastAs, LastKop, lastKep, lastEkor);
        if (dto != null) listLog.push(dto);
        if (coloring == 'CX') {
            dto = restrukturNumberHitGame(log, 'AS-KOP-EKOR', gameCode, gameId, gamePeriode, lastAs, LastKop, lastKep, lastEkor);
            if (dto != null) listLog.push(dto);
            dto = restrukturNumberHitGame(log, 'AS-KEP-EKOR', gameCode, gameId, gamePeriode, lastAs, LastKop, lastKep, lastEkor);
            if (dto != null) listLog.push(dto);
            dto = restrukturNumberHitGame(log, 'KOP-KEP-EKOR', gameCode, gameId, gamePeriode, lastAs, LastKop, lastKep, lastEkor);
            if (dto != null) listLog.push(dto);
        } else {
            dto = restrukturNumberHitGame(log, 'KOP-KEP-EKOR', gameCode, gameId, gamePeriode, lastAs, LastKop, lastKep, lastEkor);
            if (dto != null) listLog.push(dto);
    ...
    $.ajax({
        type: "POST",
        url: "/ApiQueue/NewSaveAnalisa",
        data: JSON.stringify(listLog),
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: async function (data) {
            await showToastr('Set Range For BUY & SALE SUKSES', 'SET NOT RANGE BUY & SALE', 'FROM SERVER');
        },
        error: function (req, status, error) {
            console.log(error);
        }
...
async function autoSaveDataLogInChildChart(numbering) {
    ...
    let listLog = [];
    let dto = restrukturNumberHitGame(numbering, 'AS-KOP-KEP', gameCode, gameId, gamePeriode, lastAs, LastKop, lastKep, lastEkor, 'xx');
    if (dto != null) listLog.push(dto);

    dto = restrukturNumberHitGame(numbering, 'AS-KOP-EKOR', gameCode, gameId, gamePeriode, lastAs, LastKop, lastKep, lastEkor, 'xx');
    if (dto != null) listLog.push(dto);

    dto = restrukturNumberHitGame(numbering, 'AS-KEP-EKOR', gameCode, gameId, gamePeriode, lastAs, LastKop, lastKep, lastEkor, 'xx');
    if (dto != null) listLog.push(dto);
    dto = restrukturNumberHitGame(numbering, 'KOP-KEP-EKOR', gameCode, gameId, gamePeriode, lastAs, LastKop, lastKep, lastEkor, 'xx');
    if (dto != null) listLog.push(dto);
...
window.restrukturNumberHitGame = function (log, typePick, gameCode, gameId, gamePeriode, lastAs, LastKop, lastKep, lastEkor, tipePick) {
    var asx = '';
    var kop = '';
    var kepala = '';
    var ekor = '';
    if (tipePick !== undefined && tipePick !== null) {
        asx = log.substring(0, 1);
        kop = log.substring(1, 2);
        kepala = log.substring(2, 3);
        ekor = log.substring(3, 4);
    } else {
        asx = log.logResult.substring(0, 1);
        kop = log.logResult.substring(1, 2);
        kepala = log.logResult.substring(2, 3);
        ekor = log.logResult.substring(3, 4);
    }

    if (asx == lastAs && kop == LastKop) return null;
    else if (kop == LastKop && kepala == lastKep) return null;
    else if (kepala == lastKep && ekor == lastEkor) return null;


    let A1 = asx.toString() + '-' + kop.toString() + '-' + kepala.toString();
    let A2 = asx.toString() + '-' + kop.toString() + '-' + ekor.toString();
    let A3 = asx.toString() + '-' + kepala.toString() + '-' + ekor.toString();
    let A4 = kop.toString() + '-' + kepala.toString() + '-' + ekor.toString();

    let numb = '';
    if (typePick == 'AS-KOP-KEP') numb = A1;
    else if (typePick == 'AS-KOP-EKOR') numb = A2;
    else if (typePick == 'AS-KEP-EKOR') numb = A3;
    else numb = A4;
